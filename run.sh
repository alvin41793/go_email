...
if [ $? -eq 0 ]; then
    ...
    if [ "$can_run" = true ] && [ "$GOOS" != "windows" ]; then
        read -p "是否运行程序? (1:前台运行, 2:后台运行, 3:不运行) [默认2]: " run_choice
        run_choice=${run_choice:-2}
        ...
        case $run_choice in
            1)
                echo "在前台运行程序(${env_name}环境)..."
                $output_file -env $env_name
                ;;
            2)
                echo "在后台运行程序(${env_name}环境)..."
                echo "日志输出到: $log_file"
                nohup $output_file -env $env_name > $log_file 2>&1 &
                echo "程序已在后台启动，进程ID: $!"
                ;;
            *)
                echo "已跳过运行程序"
